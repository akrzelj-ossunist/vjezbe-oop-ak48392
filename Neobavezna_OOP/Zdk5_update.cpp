...
void comp_vs_player_hard(vector<int>v,int &win)
{
	int num = 0;
	srand((int)time(NULL));
	int br = rand() % 2;
	while (v.size())
	{
		if (br == 0 && (v.size() - 1) % 4 == 0)
		{
			cout << "Computer picked 1..." << endl;
			v.pop_back();
		}
			
		else if (br == 0 && (v.size() - 1) % 2 == 0)
		{
		...
		else if (br == 0 && v.size() % 2 == 0 && v.size() % 4 != 0)
		{
			cout << "Computer picked 2..." << endl;
			v.pop_back();
			v.pop_back();
		}
		else if (br == 0 && v.size() % 4 == 0)
		{
			cout << "Computer is choosing how many matches he want... " << endl;
			int ran = rand() % 3 + 1;
			cout << "He picked " << ran << "..." << endl;
...
void main()
{
	...
	cin >> diff;
	if (diff == 'e')
		comp_vs_player_easy(v, Win);
	else if (diff == 'n')
		comp_vs_player_normal(v, Win);
	else if (diff == 'h')
		comp_vs_player_hard(v, Win);
	else
