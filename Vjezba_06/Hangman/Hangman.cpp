...
void Hangman_Controller::displayHangman(Hangman_Model M)
{
	if (M.miss_cnt == 8)
		return;
	if (M.miss_cnt == 7)
...
void Hangman_Controller::displayProgress(Hangman_Model M)
{
	cout << "Current state of your sentence is: " << M.str_new << endl;
}
void Hangman_Controller::displayUsedLetters(char c, int it, string &usedLettersC, string &usedLettersF)
{
	int brC = 0, brF = 0;
...
void Hangman_View::updateLives(Hangman_Model M)
{
	cout << "\nYou have " << M.miss_cnt << " lives left!" << endl;
}

void Hangman_View::userEntry(char &c)
{
...
void Hangman_View::checkLetter(char c,Hangman_Model &M, int &it)
{
	char cc;
	it = M.str.find(c);
	if (it < 0)
...
void Hangman_View::gameStatus(Hangman_Model M, char &c)
{
	if (M.str == M.str_new)
...
void main()
{
	...
	while (fp)
	{
		...
		for (M.miss_cnt; M.miss_cnt != 0 && M.str != M.str_new;)
		{
			V.updateLives(M);
			V.userEntry(c);
			V.checkLetter(c, M, it);
			C.displayProgress(M);
			C.displayUsedLetters(c, it, usedLettersC, usedLettersF);
			C.displayHangman(M);
			V.gameStatus(M, c);
		}
		if (c == 'y')
